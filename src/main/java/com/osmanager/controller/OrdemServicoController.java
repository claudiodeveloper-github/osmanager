...
import com.osmanager.service.PdfService;
...
public class OrdemServicoController {
    ...
    public String editar(@PathVariable Long id, Model model) {
        model.addAttribute("os", ordemServicoService.buscarPorId(id));
        model.addAttribute("clientes", clienteRepository.findAll());
        model.addAttribute("equipamentos", equipamentoRepository.findAll());
        model.addAttribute("statusList", Status.values());
        return "os/form";
    ...
    public String atualizar(@ModelAttribute OrdemServico os) {
        ordemServicoService.atualizarStatus(os.getId(), os.getStatus());
        return "redirect:/os";
    }
    ...
    public void exportarPdf(@PathVariable Long id, HttpServletResponse response) throws IOException {
        ...
        response.setHeader("Content-Disposition", "attachment; filename=os_" + id + ".pdf");

        OrdemServico os = ordemServicoService.buscarPorId(id);
        pdfService.gerarPdfOs(os, response);
    }
}